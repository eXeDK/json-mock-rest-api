    ...
    return function(req, res, next) {
        ...
        _.keys(endpointValidationRules).forEach((elementName) => {
            ...
            if (typeof elementValidationRules === 'string') {
                ...
                return next(new restify.errors.InternalError('Validation data not correct'));
            }
        });
        return next();

        function validateSingleInput(validationName, elementName, expectedValue) {
            var validationResult = require('./validationRules/' + validationName)(elementName, req.body[elementName], expectedValue);
